...
(def DOCS-CHANGES-INFO "docs-changes-info")

(defn get-modification-info [indexes json1 json2]
...
(defn diff-event-processor [indexes [json1 json2]]
  (cond
   (and (nil? json1) (nil? json2)) {:zen 1}
   (nil? json1) {:event-added 1}
   (nil? json2) {:event-deleted 1}
   :else (let [info (get-modification-info indexes json1 json2)]
           (conj info
                 (if (empty? info)
  ...
  (let [indexes (into #{} (read-index-map))]
    (with-open [rdr (reader file)]
      (->> rdr
           ...
           (map verify-event)
           print-state-info
           (map (partial diff-event-processor indexes))
           (apply merge-with +)
           show
           (spit "docs-info")))))
